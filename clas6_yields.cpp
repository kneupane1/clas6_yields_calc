...
THnSparseD *h_exp_prot_7D;
THnSparseD *h_exp_pip_7D;
THnSparseD *h_exp_pim_7D;
...
int main(int argc, char **argv)
{
  ...
  std::string inFileName = "/Users/krishnaneupane/Desktop/2021/oct_2021/e16_yield_tests_7D.root";
  ...
  for (short q2 = 3; q2 < 4; q2++) {
    ...
    //   std::cout << " w_bin for flux " << w_bin_for_flux << "\n";
      ...
      auto name = Form("h_5dim_%.1f<=Q2<=%.1f GeV2_%.3f<=W<=%.3f GeV", 3.0, 3.5, 1.4, 2.125);
      auto output_name = Form("h_1D_%.1f<=Q2<=%.1f GeV2_%.3f<=W<=%.3f GeV", 3.0, 3.5, 1.7, 1.725);

        h_exp_prot_7D = (THnSparseD *)root_data->Get(Form("THnSparse_7D_prot/%s", name));

        // THnSparse::GetAxis(12)->SetRange(from_bin, to_bin);
        // TH1D *yield_inv_Ppip = h_exp_prot_7D->GetAxis(0)->SetRange();
        // 5 Projection and Normalization : 3 factors
        // h_exp_prot_7D->GetAxis(0)->SetRange(12, 13);  // bin ko range ho x ra y axis ko range hoina hy
        h_exp_prot_7D->GetAxis(1)->SetRange(0, 1);
        TH1D *W_hist_7D = h_exp_prot_7D->Projection(0);
        W_hist_7D->SetTitle("W_hist_from_THnSparse");
        W_hist_7D->SetXTitle("W (GeV)");
        W_hist_7D->Draw("L");
int estimate;
// estimate =  h_exp_prot_7D->Projection(0)->Integral(0, 29);
estimate = W_hist_7D->Integral(0, 29);

std::cout <<"total int is: "<< estimate << "\n";

// can1->SaveAs(Form("mPim_clas6_from_7D_%s.png", output_name));
        }


        //   auto output_name = Form("nine_1D_cs_%.1f<=Q2<=%.1f GeV2_%.3f<=W<=%.3f GeV", (q2_lower_lim),
        //   (q2_upper_lim),
        //                           (1.0 + 0.05 * w), (1.0 + 0.05 * w + 0.05));
        //   // auto output_name = Form("nine-1D_cs_%.1f<=Q2<=%.1f GeV2_%.3f<=W<=%.3f GeV", (q2_lower_lim),
        //   (q2_upper_lim),
