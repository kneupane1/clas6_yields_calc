...
Histogram::Histogram(const std::string& output_file) {
  ...
  //   // std::cout << "q2 is " << q2 << " low lim = " << q2_low_values[q2] << " up lim = " << q2_up_values[q2]
  //   //           << std::endl;
  ...
  //   for (short w = 0; w < 8; w++) {
  //     // Mupper(W) = W − mh3
  //     Double_t xmax_5D[ndims_5D] = {(1.4 + 0.05 * w - MASS_PIM), (1.4 + 0.05 * w - MASS_P), 180, 360, 360};
      
      
  //     auto name = Form("h_5dim_%.1f<=Q2<=%.1f GeV2_%.3f<=W<=%.3f GeV", 3.0, 3.5, (1.4 + 0.05 * w),
  //                      (1.4 + 0.05 * w + 0.05));

  //     sevenDHist_prot[w] = new THnSparseD(name, name, ndims_5D, bins_5D, xmin_5D, xmax_5D);
  //     sevenDHist_prot[w]->Sumw2();

      
  //     sevenDHist_pip[w] = new THnSparseD(name, name, ndims_5D, bins_5D, xmin_5D, xmax_5D);

  //     sevenDHist_pip[w]->Sumw2();


  //     sevenDHist_pim[w] = new THnSparseD(name, name, ndims_5D, bins_5D, xmin_5D, xmax_5D);

  //     sevenDHist_pim[w]->Sumw2();
  ...
  Double_t xmin_5D[ndims_5D] = {1.4,2.0, 1.1, 0.3, 0., 0., 0.};
    ...
    auto name =
        Form("h_5dim_%.1f<=Q2<=%.1f GeV2_%.3f<=W<=%.3f GeV", 3.0, 3.5, 1.4 , 2.125);
...
void Histogram::Fill_W_bin_check(const std::shared_ptr<Reaction>& _e) {
  ...
  if (_e->MM_cut()) {
    if (_e->W() <= 1.8 && _e->W() >= 1.4 && _e->Q2() >= 3.0 && _e->Q2() <= 3.5) {
