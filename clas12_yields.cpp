...
{
  std::string inFileName = "/Users/krishnaneupane/Desktop/2021/oct_2021/rga_bin_filling_checks.root";
  ...
  for (short q2 = 3; q2 < 4; q2++) {
    ...
    for (short w = 15; w < 16; w++) {
      ...
      auto name = Form("h_5dim_%.1f<=Q2<=%.1f GeV2_%.2f<=W<=%.2f GeV", (q2_lower_lim), (q2_upper_lim), (1.0 + 0.05 * w),
                       (1.0 + 0.05 * w + 0.05));

      auto output_name = Form("nine_1D_cs_%.1f<=Q2<=%.1f GeV2_%.2f<=W<=%.2f GeV", (q2_lower_lim), (q2_upper_lim),
...

      {
        ...
        h_exp_prot[no_w_bins]->Scale(1. / 0.0379);
      ...
      {
        h_exp_pip[no_w_bins]->Scale(1. / 0.0379);
      ...
      {
        // faraday cup charge = 0.038 ..C
        h_exp_pim[no_w_bins]->Scale(1. / 0.0379);
      ...
      // // output_png->cd();
      can1->SaveAs(Form("mPim_yiels_clas12_check_%s.png", output_name));
    }
